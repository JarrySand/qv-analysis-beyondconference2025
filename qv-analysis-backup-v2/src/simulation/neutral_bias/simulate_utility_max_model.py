...
"""
効用最大化仮説モデルのシミュレーションと実際の投票分布の比較分析
"""
...
def simulate_optimal_votes_with_zero(n_simulations=1000, n_projects=7, max_credits=99, 
    ...
    """
    Simulate optimal votes based on utility maximization, including the possibility of zero votes
    ...
    indifference_threshold : float
        Threshold below which a voter is indifferent (will choose 0 votes)
    decision_cost : float
        The cognitive cost of deciding to vote for a project
    ...
    print(f"  デバッグ: シミュレーション開始 (閾値={indifference_threshold}, コスト={decision_cost})")
    ...
    for sim in range(n_simulations):
        ...
        # If preference < threshold + decision_cost, the voter won't vote (0 votes)
        adjusted_preferences = np.copy(preferences)
        for i in range(len(adjusted_preferences)):
            if adjusted_preferences[i] < indifference_threshold + decision_cost:
                adjusted_preferences[i] = 0
            else:
                # Subtract decision cost from preference
                adjusted_preferences[i] -= decision_cost
        
        # Create array for final votes
        final_votes = np.zeros(n_projects)
        
        # Filter out projects with zero preferences
        positive_indices = np.where(adjusted_preferences > 0)[0]
        positive_preferences = adjusted_preferences[positive_indices]
        
        if len(positive_preferences) > 0:
            # Calculate effective max credits based on the number of projects being voted for
            # This is a simplification to represent focused spending on fewer projects
            effective_max_credits = max_credits
            
            # Optimal voting calculation for positive preferences
            sum_squared = np.sum(positive_preferences**2)
            if sum_squared > 0:  # Avoid division by zero
                denominator = np.sqrt(sum_squared / effective_max_credits)
                optimal_votes_positive = positive_preferences / denominator
                
                # Discretize votes (round to 1-9)
                discretized_votes_positive = np.clip(np.round(optimal_votes_positive), 1, 9)
                
                # Assign back to final votes array
                for idx, original_idx in enumerate(positive_indices):
                    final_votes[original_idx] = discretized_votes_positive[idx]
        
        # Add all votes to results
        simulated_votes.extend(final_votes)
        
        # デバッグ出力（最初の数回のシミュレーションのみ）
        if sim < 3:
            print(f"    シミュレーション {sim+1}: 選好値 = {preferences.round(2)}")
            print(f"    シミュレーション {sim+1}: 調整後選好値 = {adjusted_preferences.round(2)}")
            print(f"    シミュレーション {sim+1}: 最終投票値 = {final_votes.astype(int)}")
    ...
    # デバッグ情報
    print(f"  デバッグ: 生成された投票総数: {len(simulated_votes)}")
    print(f"  デバッグ: 投票値の種類: {sorted(vote_counts.index.tolist())}")
    print(f"  デバッグ: 0票の割合: {vote_percentages.get(0, 0):.2f}%")
...
def main():
    print("効用最大化仮説のシミュレーション分析を開始...")
    
    # データ読み込み
    votes_df, candidates_df = load_data()
    print(f"データ読み込み完了: {len(votes_df)}件の投票データ, {len(candidates_df)}件のプロジェクト")
    
    # 実際の投票値分布を取得
    actual_counts, actual_percentages = get_vote_distribution(votes_df)
    print("実際の投票値分布:")
    for i, pct in actual_percentages.items():
        print(f"  {i}票: {pct:.2f}%")
    
    # アプローチ①: 実データから0票以下を除外した分布を取得
    print("\nアプローチ①: 実データから0票以下を除外した分析")
    filtered_counts, filtered_percentages = get_filtered_vote_distribution(votes_df, min_value=1)
    print("0票以下を除外した実際の投票値分布:")
    for i, pct in filtered_percentages.items():
        print(f"  {i}票: {pct:.2f}%")
    
    # 従来の効用最大化モデルによるシミュレーション
    print("\n従来の効用最大化モデルシミュレーション:")
    simulated_counts, simulated_percentages = simulate_optimal_votes(
        n_simulations=1000, 
        n_projects=len(candidates_df),
        max_credits=99,
        preference_distribution='uniform',
        params={'min': 0, 'max': 10}
    )
    
    print("従来の効用最大化モデルの理論分布:")
    for i, pct in simulated_percentages.items():
        print(f"  {i}票: {pct:.2f}%")
    
    # 分布の比較 (従来モデルvs全データ)
    comparison_df = compare_distributions(actual_percentages, simulated_percentages)
    print("\n従来モデルvs全データの比較:")
    print(comparison_df)
    
    # 統計的検定 (従来モデルvs全データ)
    chi2_stat, p_value, df = chi_square_test(actual_counts, simulated_counts)
    print(f"従来モデルvs全データのカイ二乗検定: χ²={chi2_stat:.4f}, df={df}, p={p_value:.8f}")
    
    # 分布の比較 (従来モデルvsフィルタリングデータ)
    filtered_comparison_df = compare_distributions(filtered_percentages, simulated_percentages)
    print("\n従来モデルvsフィルタリングデータの比較:")
    print(filtered_comparison_df)
    
    # 統計的検定 (従来モデルvsフィルタリングデータ)
    filtered_chi2_stat, filtered_p_value, filtered_df = chi_square_test(filtered_counts, simulated_counts)
    print(f"従来モデルvsフィルタリングデータのカイ二乗検定: χ²={filtered_chi2_stat:.4f}, df={filtered_df}, p={filtered_p_value:.8f}")
    
    # 可視化 (従来のモデル)
    # 1. 従来の効用最大化モデルvs全データ
    plot_distribution_comparison(comparison_df, f"{OUTPUT_DIR}/utility_max_comparison.png")
    
    # 2. 従来の効用最大化モデルvsフィルタリングデータ
    plot_distribution_comparison(filtered_comparison_df, f"{OUTPUT_DIR}/filtered_utility_max_comparison.png")
    
    # レポート生成 (従来のモデル)
    # 1. 従来の効用最大化モデルvs全データ
    generate_report(votes_df, actual_counts, actual_percentages, 
                   simulated_counts, simulated_percentages, comparison_df,
                   chi2_stat, p_value, df, f"{OUTPUT_DIR}/utility_max_report.md")
    
    # 2. 従来の効用最大化モデルvsフィルタリングデータ
    generate_report(votes_df, filtered_counts, filtered_percentages, 
                   simulated_counts, simulated_percentages, filtered_comparison_df,
                   filtered_chi2_stat, filtered_p_value, filtered_df, 
                   f"{OUTPUT_DIR}/filtered_utility_max_report.md")
    
    # アプローチ②: 0票を含む効用最大化モデルの実行
    print("\nアプローチ②: 0票を含む効用最大化モデル")
    # いくつかの異なるパラメータ設定でシミュレーションを実行
    thresholds = [0.5, 1.0, 1.5]
    costs = [0.1, 0.2, 0.3]
    
    best_threshold = 0.5  # デフォルト値を設定
    best_cost = 0.2  # デフォルト値を設定
    best_chi2 = float('inf')
    best_zero_model_counts = None
    best_zero_model_percentages = None
    
    # ベストモデルを評価するための指標を追加
    best_zero_percent_diff = float('inf')  # 0票の割合の差を最小化
    
    print("パラメータ最適化中...")
    for threshold in thresholds:
        for cost in costs:
            try:
                zero_model_counts, zero_model_percentages = simulate_optimal_votes_with_zero(
                    n_simulations=1000,
                    n_projects=len(candidates_df),
                    max_credits=99,
                    preference_distribution='uniform',
                ...
                # モデルの0票割合を取得
                model_zero_percent = zero_model_percentages.get(0, 0)
                # 実データの0票割合
                actual_zero_percent = actual_percentages.get(0, 0)
                # 差異の絶対値
                zero_percent_diff = abs(model_zero_percent - actual_zero_percent)
                
                print(f"  閾値={threshold}, コスト={cost}: 0票割合={model_zero_percent:.2f}% (実データ: {actual_zero_percent:.2f}%, 差異: {zero_percent_diff:.2f}%)")
                
                # 統計的検定も試みる
                try:
                    zero_chi2_stat, zero_p_value, zero_df = chi_square_test(actual_counts, zero_model_counts)
                    print(f"    χ²={zero_chi2_stat:.4f}, p={zero_p_value:.4f}, df={zero_df}")
                except Exception as e:
                    print(f"    統計的検定でエラー: {str(e)}")
                    zero_chi2_stat = float('inf')
                
                # 0票の割合の差異に基づいて最適モデルを選択
                if zero_percent_diff < best_zero_percent_diff:
                    best_zero_percent_diff = zero_percent_diff
                    best_threshold = threshold
                    best_cost = cost
                    best_chi2 = zero_chi2_stat
                    best_zero_model_counts = zero_model_counts.copy()
                    best_zero_model_percentages = zero_model_percentages.copy()
                    print(f"    新しい最適パラメータを発見!")
            except Exception as e:
                print(f"  シミュレーションエラー (閾値={threshold}, コスト={cost}): {str(e)}")
    
    print(f"\n最適パラメータ: 閾値={best_threshold}, コスト={best_cost}, 0票の差異={best_zero_percent_diff:.2f}%, χ²={best_chi2:.4f}")
    
    # 最良のモデルが見つからなかった場合、計算を続行しない
    if best_zero_model_counts is None or best_zero_model_percentages is None:
        print("適切なパラメータが見つかりませんでした。0票モデルの分析をスキップします。")
    else:
        # 分布の比較 (0票モデルvs全データ)
        zero_comparison_df = compare_distributions(actual_percentages, best_zero_model_percentages)
        print("\n0票モデルvs全データの比較:")
        print(zero_comparison_df)
        
        # 統計的検定 (0票モデルvs全データ)
        try:
            zero_chi2_stat, zero_p_value, zero_df = chi_square_test(actual_counts, best_zero_model_counts)
            print(f"0票モデルvs全データのカイ二乗検定: χ²={zero_chi2_stat:.4f}, df={zero_df}, p={zero_p_value:.8f}")
        except Exception as e:
            print(f"0票モデルvs全データの統計的検定でエラー: {str(e)}")
        
        # 可視化
        # 3. 0票を含む効用最大化モデルvs全データ
        plot_distribution_comparison(zero_comparison_df, f"{OUTPUT_DIR}/zero_model_comparison.png")
        
        # 4. 両モデルと実データの比較
        distributions = {
            '従来の効用最大化モデル': simulated_percentages,
            '0票を含む効用最大化モデル': best_zero_model_percentages
        }
        plot_multiple_distributions(actual_percentages, distributions, 
                                  f"{OUTPUT_DIR}/model_comparison.png")
        
        # 3. 0票を含む効用最大化モデルvs全データのレポート
        try:
            generate_report(votes_df, actual_counts, actual_percentages, 
                           best_zero_model_counts, best_zero_model_percentages, zero_comparison_df,
                           zero_chi2_stat, zero_p_value, zero_df, 
                           f"{OUTPUT_DIR}/zero_model_report.md")
        except Exception as e:
            print(f"レポート生成でエラー: {str(e)}")
            # 簡易レポートを作成
            with open(f"{OUTPUT_DIR}/zero_model_report.md", 'w', encoding='utf-8') as f:
                f.write("# 0票を含む効用最大化モデル分析\n\n")
                f.write(f"最適パラメータ: 閾値={best_threshold}, コスト={best_cost}\n\n")
                f.write(f"0票の割合の差異: {best_zero_percent_diff:.2f}%\n\n")
                f.write("## 分布比較\n\n")
                f.write(zero_comparison_df.to_markdown())
