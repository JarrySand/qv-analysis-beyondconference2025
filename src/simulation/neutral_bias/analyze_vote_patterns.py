...
STATS_OUTPUT_DIR = 'results/data'  # 統計ファイルの出力先を別途指定
...
def main():
    ...
    # Save CSV files - 統計ファイルはSTATS_OUTPUT_DIRに保存
    vote_stats.to_csv(os.path.join(STATS_OUTPUT_DIR, 'vote_statistics.csv'), index=False)
    voter_stats.to_csv(os.path.join(STATS_OUTPUT_DIR, 'voter_statistics.csv'), index=True)
    
    # ビジュアライゼーション用にOUTPUT_DIRにも保存（オプション）
